...
public class BSTImplME<T extends Comparable<T>> implements BST<T> {
...
*/
	public T[] preOrder() {
		...
		Comparable[] arrayFinal = new Comparable[size()];
	
		
		array = preOrder(node,array);
		return (T[]) array.toArray(arrayFinal);
	}
	
	public ArrayList<T> preOrder(BSTNode<T> node, ArrayList<T> array) {
		if (node == null) return null;
	...
	public void visit(BSTNode<T> node, ArrayList<T> array){
		array.add(node.getData());
	}
